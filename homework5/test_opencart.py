...
def test_catalog_page(browser, url):
    ...
    card_field = WebDriverWait(browser, 3).until(EC.visibility_of_element_located((By.ID, "cart")))
    assert card_field.find_element(By.TAG_NAME, "span").text == "0 item(s) - $0.00"


def test_product_card(browser, url):
    browser.get(url + "index.php?route=product/product&path=18&product_id=47")
